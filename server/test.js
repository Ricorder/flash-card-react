  ...
  if (index === 4) {
    res.json({theme})
      ...
      if (index === 3) {
        ...
        res.json({
          question: { question: 'Сейчас мы узнаем ответ на последний вопрос', theme },
          index: index + 1,
          validation: lastCheck ? "Ответ правильный" : "Ответ неправильный",
        })
      } else {
          if (userAnswer === rightTheme[index - 1].answer) {
            result += 1;
            res.json({
              question: rightTheme[index],
              index: index + 1,
              validation: "Ответ правильный",
            })
          } else {
            res.json({
              question: rightTheme[index],
              index: index + 1,
              validation: "Ответ неправильный",
            })
          }
    }
  }
